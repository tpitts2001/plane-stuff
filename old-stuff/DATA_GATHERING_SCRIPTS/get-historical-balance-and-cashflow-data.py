...
def save_data_to_file(data, symbol, data_type, output_directory_base):
    ...
    print(f"Saved {data_type} historical-data for {symbol} to {filepath}")
    ...
    for ticker in tickers:
        ...
        for data_type in ['balance_sheet', 'cash_flow']:
            ...
            if "quarterlyReports" not in financial_data:
                print(f"No 'quarterlyReports' found in {data_type} historical-data for {ticker}. Skipping.")
                continue  # Skip saving this historical-data type for the current ticker
...
def run_at_specific_time(hour=15, minute=59):
    while True:
        ...
        file_path = "../../historical-data/airline_identifiers/tickers_cik.csv"
        output_directory_base = "../historical-data/temp"
